...
RedMage.AliasList = T{'stone','water','aero','fire','blizzard','thunder','Zz','refresh','haste','enstone','enwater','enaero','enfire','enblizzard','enthunder','mpset','eva','pdt','mdt'};
...
function RedMage.HandleCommands(args)
    ...
    elseif (args[1] == 'mdt') then
            if (RedMage.Settings.MDT == true) then
            RedMage.Settings.MDT = false;
            AshitaCore:GetChatManager():QueueCommand(1, '/echo MDT Mode De-Activated...');
        else
            RedMage.Settings.MPSET = false;
            RedMage.Settings.EVA = false;
            RedMage.Settings.PDT = false;
            RedMage.Settings.MDT = true;
            AshitaCore:GetChatManager():QueueCommand(1, '/echo MDT Mode Activated!');
        end
    elseif (args[1] == 'pdt') then
    if (RedMage.Settings.PDT == true) then
            RedMage.Settings.PDT = false;
            AshitaCore:GetChatManager():QueueCommand(1, '/echo PDT Mode De-Activated...');
        else
            RedMage.Settings.MPSET = false;
            RedMage.Settings.EVA = false;
            RedMage.Settings.PDT = true;
            RedMage.Settings.MDT = false;
            AshitaCore:GetChatManager():QueueCommand(1, '/echo PDT Mode Activated!');
        end
    end
end
